...
> Threat-Intelligence - Open source tool by AetherCodeHQ

`cli` `golang` `security` `threat-intelligence`

## What is it?

**Threat Intelligence** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Threat-Intelligence.git
cd Threat-Intelligence
go build -o Threat-Intelligence .
```

### Run
```bash
./Threat-Intelligence --help
```
...
```bash
# Basic usage
./Threat-Intelligence --path ./target

# With options
./Threat-Intelligence --path ./target --format json --output report.json

# Verbose mode
./Threat-Intelligence --path ./target --verbose
```
...
### Basic scan
```bash
./Threat-Intelligence --path ./my-project
```

### JSON report
```bash
./Threat-Intelligence --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Threat Intelligence
  run: ./Threat-Intelligence --path . --format json --output report.json
```
